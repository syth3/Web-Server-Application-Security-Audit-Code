...
def verify_method(method, parsed_request):
    ...
    Set parsed_request._http_method to 501 if it is not.
...
def verify_request_uri(request_uri, parsed_request):
    """Verify that the request URI is valid. Set parsed_request._response_code to 400 if it is not.
    Note: This function was not needed for this assignment.
          It will be implemented at a later time if/when it is needed
...
def verify_http_version(http_version, parsed_request):
    """Verify that HTTP 1.1 or HTTP 1.0 is used. Set parsed_request._response_code to 505 if it is not.
...
def verify_request_line(request_line, parsed_request):
    ...
    Set parsed_request._response_code to 400 if it is not valid.
...
def verify_headers(headers, parsed_request):
    """Verify that all headers are syntaxually correct and that the "Host" header is included if HTTP/1.1 is being used.
    Set parsed_request._response_code to 400 if there is no "Host" header and HTTP/1.1 is being used
...
def parse_request(request):
    """Parse a HTTP request and check for syntax. If syntax is incorrect, respond with a 400 HTTP code.
    Otherwise, respond with a 200 HTTP code.
    
    Parameters
    ----------
    request : string
        HTTP request read as a byte stream, converted to a string, and b'' removed
        Example: if the request after being converted to a string looks like this:
          b'string here', remove the b' from the beginning and ' at the end before
